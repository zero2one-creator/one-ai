...
- 📝 **搜索历史** - 自动保存搜索记录，方便快速查看和复用历史问题，所有数据都存储在本地
- ⚡ **One Command** - 预设常用 prompt 模板，输入 `/` 快速搜索并填充到搜索框
...
│   │       │   ├── SearchInput.vue       # 搜索输入框（支持多行、展开编辑、斜杠命令）
│   │       │   ├── SplitLayout.vue       # 分屏布局（支持拖拽调整）
│   │       │   ├── HistoryModal.vue      # 历史记录弹窗
│   │       │   ├── PromptPresetModal.vue # One Command 弹窗
...
│   │   ├── promptStore.ts    # One Command 存储操作
│   │   └── historyStore.ts   # 搜索历史存储操作
│   │
│   ├── App.vue               # 根组件
│   ├── main.ts               # 入口文件
│   └── style.css             # 全局样式
